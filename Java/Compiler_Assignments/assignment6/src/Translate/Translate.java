...
public class Translate implements SyntaxTreeVisitor <LazyIRTree>{
	...
	public LazyIRTree visit(MainClass n) {
		...
		SEQ stms = SEQ.fromList(
				new LABEL("$main$" + n.i1.s),
				main.asStm()
				);
		procedureFragments.add(new ProcFrag(stms));
		return null;
	}
	...
	public LazyIRTree visit(SimpleClassDecl n) {
		currentClass = n.i.s;
		currentFrame = new NAME (NameOfLabel.generateLabel(currentClass));
		for(MethodDecl method: n.ml) {
			procedureFragments.add(new ProcFrag(method.accept(this).asStm()));
		}
		return null;
	}
	...
	public LazyIRTree visit(If n) {
		...
		return new IfThenElseExp (conditional, thenStatements.asExp(), elseStatements.asExp());
	}
	...
	public LazyIRTree visit(Print n) {
		...
		SEQ stms = SEQ.fromList(
				new EVAL(new CALL(new NAME("_print"))),
				printExpression.asStm()
				);
		return new StatementIRTree(stms);
	}
	...
	public LazyIRTree visit(Assign n) {
		...
		if(localNumber != -1) {
			...
			return new ExpressionIRTree(memMove);
		}
		return null;
	}
	...
	public LazyIRTree visit(Call n) {
		TEMP objectCallTemp = TEMP.generateTEMP();
		LazyIRTree expDetails = n.e.accept(this);
		TEMP expTemp = TEMP.generateTEMP();
		MOVE expMove = new MOVE (expTemp, expDetails.asExp());
		NAME callName = new NAME(n.i.s);
		ExpList parameters = convertToExpList(n.el);
		ExpList fullParameters = new ExpList (expTemp, parameters);
	...
	public LazyIRTree visit(NewObject n) {
		...
		if(classObject.extendsParentClass()) {
			...
			allocationCall = new CALL(new NAME("_allocate_object"), new CONST(totalFields));
		}
		else {
			allocationCall = new CALL(new NAME("_allocate_object"), new CONST(childFields));
		}
